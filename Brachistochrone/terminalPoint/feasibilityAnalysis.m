...
%We interpolate sin(theta) and cos(theta) rather than theta directly, since theta has the
%modulo 2pi issue, and so can be quite sharp (nonSmooth).  We fix the nonSmoothness via
%interpolating off of sin and cos instead.
uCosTheta = @(t) interp1(tNodes, cos(uOpt), t);
uSinTheta = @(t) interp1(tNodes, sin(uOpt), t);
%Here, x is our state vector: x(1) is x, x(2) is y and x(3) is v.  Note 'g' is defined in
%the main file
system = @(t, x) [ x(3)*uSinTheta(t); x(3)*uCosTheta(t); g*uCosTheta(t)];
